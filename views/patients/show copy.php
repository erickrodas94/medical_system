            ...
            <div class="lg:col-span-1 space-y-2">
                ...
                <?php if (($_SESSION['user']['specialty'] ?? '') === 'specialty_pediatrics'): ?>
                    <button onclick="switchTab('tab-pediatrics')" id="btn-tab-pediatrics" class="tab-btn w-full flex items-center px-4 py-3 bg-white border border-slate-100 text-slate-500 hover:bg-slate-50 hover:text-slate-700 rounded-xl font-semibold transition-all shadow-sm">
                        <i data-lucide="baby" class="w-5 h-5 mr-3 text-slate-400"></i> Crecimiento OMS
                    </button>
                <?php endif; ?>
            </div>
                ...
                <div class="bg-white border border-slate-200 rounded-2xl p-6 min-h-[400px] shadow-sm">
                    ...
                    <div id="tab-pediatrics" class="tab-content hidden">
                        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                            <h3 class="text-lg font-bold text-slate-800">Curvas de Crecimiento OMS</h3>
                            
                            <div class="flex bg-slate-100 p-1 rounded-xl border border-slate-200">
                                <button onclick="changePediatricMetric('weight')" id="btn-weight" class="metric-btn active-metric px-4 py-1.5 rounded-lg text-xs font-bold transition-all bg-white shadow-sm text-indigo-600">Peso</button>
                                <button onclick="changePediatricMetric('height')" id="btn-height" class="metric-btn px-4 py-1.5 rounded-lg text-xs font-bold transition-all text-slate-500">Talla</button>
                                <button onclick="changePediatricMetric('head')" id="btn-head" class="metric-btn px-4 py-1.5 rounded-lg text-xs font-bold transition-all text-slate-500">P. Cefálico</button>
                            </div>
                        </div>
                        ...
                        <div class="mt-4 p-4 bg-blue-50 border border-blue-100 rounded-xl text-xs text-blue-700 flex gap-3">
                            <i data-lucide="info" class="w-5 h-5 shrink-0"></i>
                            <p>Esta gráfica compara los datos del paciente con los estándares mundiales de la OMS. El carril verde (P50) representa el promedio ideal.</p>
                        </div>
                    </div>
                    <?php endif; ?>
...
<script>
    ...
    // 5. Variables para la gráfica pediátrica
    ...
    // 6. Función original para cambiar tabs
    ...
    function initGrowthChart() {
        ...
        // Obtenemos las curvas de la OMS según la métrica
        const metricKey = currentMetric === 'head' ? 'head_circumference' : currentMetric;
        const curves = whoCurves[currentMetric] || [];
        
        // Eje X (Meses)
        const labels = curves.map(c => c.age_months);
        
        // Datos del paciente filtrados
        const patientData = pediatricHistory.map(p => ({
            x: p.x,
            y: p[currentMetric]
        })).filter(p => p.y > 0);
        ...
        const config = {
            ...
            options: {
                ...
                scales: {
                    x: { title: { display: true, text: 'Edad (Meses)', font: { weight: 'bold' } } },
                    y: { title: { display: true, text: currentMetric === 'weight' ? 'Peso (kg)' : 'Centímetros (cm)', font: { weight: 'bold' } } }
                },
                plugins: {
    ...
    function initPediatricChart() {
        ...
        if (!canvas) return; // Seguridad por si el tab no existe
    ...
    // Hook para disparar la gráfica cuando se abre la pestaña
