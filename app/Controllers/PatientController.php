...
class PatientController {
    ...
    public function update() {
        ...
        try {
            ...
            $logStmt->execute([
                'cid' => $_SESSION['clinic']['id'], 'uid' => $_SESSION['user']['id'], 
                'rid' => $personId, 'ip'  => $_SERVER['REMOTE_ADDR'] ?? '0.0.0.0'
            ]);
    ...
    public function storeConsultation() {
        ...
        try {
            ...
            if ($hasInstructions || $hasMedications) {
                ...
                $prescResult = $prescriptionModel->create($prescData);
                if (!$prescResult['success']) throw new \Exception("Error al generar la receta: " . $prescResult['error']);
            }
